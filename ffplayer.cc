...
static unsigned s_resample_buf_len = 0;      // 重采样输出缓冲区长度
...
int audio_decode_frame(AVCodecContext *p_codec_ctx, AVPacket *p_packet, uint8_t *audio_buf, int buf_size)
{
    ...
    while (1)
    {
        ...
        else
        {
            ...
            /*
                p_frame->format         != s_audio_param_src.fmt            ||
            ...
            */
            ...
            if (!is_initial)
            {
                // p_frame->channel_layout = p_codec_ctx->channel_layout;
                // p_frame->channels = av_get_channel_layout_nb_channels(p_codec_ctx->channel_layout);
...
int main(int argc, char *argv[])
{
    ...
    AVFormatContext 是一个存储音视频格式相关信息的结构体，它包含了音视频流的封装格式、元数据等信息。
    p_fmt_ctx 是一个指向 AVFormatContext 结构体的指针，初始化为 NULL。
    AVCodecContext* p_codec_ctx = NULL;

    AVCodecContext 是一个存储编解码器相关信息的结构体，包含了编解码器的参数、状态等信息。
    p_codec_ctx 是一个指向 AVCodecContext 结构体的指针，初始化为 NULL。
    AVCodecParameters* p_codec_par = NULL;

    AVCodecParameters 存储了音视频编解码器的参数，包括采样率、通道数、编码类型等。
    p_codec_par 是一个指向 AVCodecParameters 结构体的指针，初始化为 NULL。
    AVCodec* p_codec = NULL;

    AVCodec 是一个表示编解码器的结构体，包含了编解码器的各种信息。
    p_codec 是一个指向 AVCodec 结构体的指针，初始化为 NULL。
    AVPacket* p_packet = NULL;

    AVPacket 用于存储压缩编码的数据，例如音视频帧。
    p_packet 是一个指向 AVPacket 结构体的指针，初始化为 NULL。
    */
    ...
    ret = avformat_open_input(&p_fmt_ctx, "rtsp://zlm.robot.genius-pros.com:18554/virtual-robot/900106-900106-573c887e-d19e-47e9-8040-3d1fad20edc8", NULL, NULL);
    if (ret != 0)
    {
    ...
    // std::cout << "channel_layout = " <<  s_audio_param_tgt.channel_layout << std::endl;
    // std::cout << "freq = " <<  s_audio_param_tgt.freq << std::endl;
    // std::cout << "fmt = " <<  s_audio_param_tgt.fmt << std::endl;
    
    if (s_audio_param_tgt.bytes_per_sec <= 0 || s_audio_param_tgt.frame_size <= 0)
    {
        std::cout <<"s_audio_param_tgt.fmt = " <<actual_spec.format <<std::endl;
        printf("av_samples_get_buffer_size failed\n");
        goto exit4;
    }
